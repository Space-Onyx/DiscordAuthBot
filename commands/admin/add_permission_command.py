...
async def add_permission_command(ctx, username: str, title: str, permission: str, server: str = "astra"):
    name_db = server.lower()
    if name_db not in ("astra", "dev"):
        await ctx.send("Неверный сервер: astra или dev")
        return

    if await ss14_db.get_admin_permission(username, db_name=name_db):
        await ctx.send(f"Пользователь {username} уже имеет права на {name_db.upper()}")
        return
