...
async def admin_command(ctx, nickname: str):
    astra_info = await ss14_db.get_admin_permission(nickname, 'astra')
    dev_info = await ss14_db.get_admin_permission(nickname, 'dev')

    if not astra_info and not dev_info:
    ...
    if astra_info:
        embed.add_field(name="ASTRA", value=f"**Титул:** {astra_info[0]}\n**Ранг:** {astra_info[1]}", inline=False)

    if astra_info and dev_info:
