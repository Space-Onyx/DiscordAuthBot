...
from dataConfig import ADDRESS_ASTRA, POST_ADMIN_HEADERS, ROLE_ACCESS_ADMIN
...
'''Команда для получения подробной статистики сервера ASTRA'''
@has_any_role(*ROLE_ACCESS_ADMIN)
@bot.command(name="admin_info")
async def admin_info_command(ctx):
    url = f"http://{ADDRESS_ASTRA}:1616/admin/info"
